...
map<uint64_t,int> block_reuse_counter,reuse_distance_counter,block_last_access_counter,block_access_counter,reuse_miss_counter,block_miss_counter;
vector<pair<uint64_t,int> > set_access[NO_OF_SETS];
set<uint64_t> dummy_hit_blocks;
set<pair<int,uint64_t> > top_misses;
...
void print_roi_stats(uint32_t cpu, CACHE* cache)
{
  ...
  if (TOTAL_ACCESS > 0) {
    ...
    if(cache->NAME=="LLC"){
      //print_reuse_stats();
      //cout<<"No_of_blocks_given_a_dummy_hit"<<" "<<dummy_hit_blocks.size()<<endl;
    }
  }
  ...
  while (std::any_of(std::begin(simulation_complete), std::end(simulation_complete), std::logical_not<uint8_t>())) {
    ...
    for (std::size_t i = 0; i < ooo_cpu.size(); ++i) {
      ...
      if (show_heartbeat && (ooo_cpu[i]->num_retired >= ooo_cpu[i]->next_print_instruction)) {
        ...
        if(EPOCH_REFRESH){
          top_misses.clear();
          block_miss_counter.clear();
          cout<<"TOPK counter refreshed"<<endl;
        }
