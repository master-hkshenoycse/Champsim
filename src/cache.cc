...
extern map<uint64_t,int> reuse_distance_counter,block_last_access_counter,block_access_counter,reuse_miss_counter,block_miss_counter,block_reuse_counter;
extern std::vector<pair<uint64_t,int> > set_access[2048];
extern std::set<uint64_t> dummy_hit_blocks;
extern int LLC_access_counter;
extern set<pair<int,uint64_t> > top_misses;

#define DUMMY_HITS_REUSE 0
#define REUSE_LOW 20
#define REUSE_HIGH 30
#define DUMMY_HITS_TOPK 0
#define TOPK_SIZE 20000
...
void CACHE::handle_read()
{
  while (reads_available_this_cycle > 0) {
    ...
    if(NAME=="LLC" and warmup_complete[handle_pkt.cpu] and handle_pkt.type==LOAD){
      ...
      if(block_access_counter[block_address]>1 and DUMMY_HITS_REUSE==1){
        int avg_reuse=block_reuse_counter[block_address]/(block_access_counter[block_address]-1);
        if(avg_reuse>=REUSE_LOW and avg_reuse<=REUSE_HIGH){
          is_hit=1;//giving a dummy hit if avg reuse falls in a range
        }
      }

      if(DUMMY_HITS_TOPK){
        if(is_hit==0){
          if(top_misses.find({block_miss_counter[block_address],block_address}) != top_misses.end()){
            top_misses.erase({block_miss_counter[block_address],block_address});
          }

          block_miss_counter[block_address]++;
          top_misses.insert({block_miss_counter[block_address],block_address});
          
          if(top_misses.size() > TOPK_SIZE){
            top_misses.erase(top_misses.begin());
          }


          //Give a dummy hit if the current miss is top miss
          if(top_misses.find({block_miss_counter[block_address],block_address}) != top_misses.end()){
            is_hit=1;
          }
