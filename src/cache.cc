...
#define predictor_ways 400
#define tag_bits 10
//In exculusive LLC is filled via the writebackqeueu only
//handle fill function will skip the fill to LLC .

extern VirtualMemory vmem;
extern uint8_t warmup_complete[NUM_CPUS];
extern std::array<CACHE*, NUM_CACHES> caches;

//@Hari Data-structures for reuse distances
extern map<uint64_t,int> reuse_distance_counter,block_last_access_counter,block_access_counter,block_reuse_counter,l2c_block_access;
extern std::vector<pair<uint64_t,int> > set_access[2048];

extern int LLC_access_counter;

extern std::vector< vector<uint64_t> > l2c_set_access[1024];
extern map<int,uint64_t> l2c_event_counter;
extern int L2C_access_counter;
extern deque<pair<int,int> > counters[predictor_sets];
extern int predictor_hits,predictor_access,actual_reuse,predicted_reuse;

//@Hari for reuse predictor 
void increment_block(int tag,int set){
      deque<pair<int,int> >::iterator it;
      
      bool is_found=0;
      for(it=counters[set].begin();it!=counters[set].end();it++){
        if((*it).first==tag){
          pair<int,int> to_insert={tag,(*it).second+1};
          counters[set].erase(it);
          is_found=1;
          counters[set].push_front(to_insert);
          break;
        }
      }
      ...
      if(is_found==0){
        //following LRU 
        if(counters[set].size()==predictor_ways){
          counters[set].pop_back();
        }

        counters[set].push_front(make_pair(tag,1));
      }
}
int get_block_count(int tag,int set){
      deque<pair<int,int> >::iterator it;
      predictor_access++;
...
int get_reuse_tag(uint64_t block_address){
    int tag=0;
    for(int i=16;i<=26;i++){
      if(block_address & (1<<i)){
        tag=tag+(1<<((i-16)));
      }
    }
...
int get_reuse_set(uint64_t block_address){
...
bool check_reuse(uint64_t block_address){
  int reuse_tag=get_reuse_tag(block_address);
  int reuse_set=get_reuse_set(block_address);
  
  int cnt=get_block_count(reuse_tag,reuse_set);
  /*if(cnt>1){
      cout<<cnt<<" "<<l2c_block_access[block_address]<<endl;
  }*/
  //cout<<block_address<<" "<<reuse_tag<<" "<<reuse_set<<" "<<cnt<<endl;
  if(l2c_block_access[block_address]>1){
    actual_reuse++;
    if(cnt>1){
      predicted_reuse++;
    }
...
void set_reuse(uint64_t block_address){
  int reuse_tag=get_reuse_tag(block_address);
  int reuse_set=get_reuse_set(block_address);
  increment_block(reuse_tag,reuse_set);
}

//@Hari to get resue distance of an access
int get_reuse(int set_no,int counter){
  std::set<uint64_t> tmp;
  
  for(auto it:set_access[set_no]){
...
//@Hari to get resue distance of an l2c access
int get_reuse_l2c(int set_no,int counter){
  std::set<uint64_t> tmp;
  
  for(auto it:l2c_set_access[set_no]){
    if(it[0] > counter and it[2]==0){
      tmp.insert(it[1]);
    }
  } 
...
//@Hari to check if address present in a cache
...
void CACHE::handle_fill()
{
  while (writes_available_this_cycle > 0) {
    ...
    uint64_t victim_block_address=victim_address>>LOG2_BLOCK_SIZE;
    ...
    if(MAKE_EXCLUSIVE){
       ...
       if(check_string(NAME,"LLC")){
        ...
        //@Hari putting the eviction from L2C to LLC write back queue for fill to LLC
        if(check_string(NAME,"L2C") or check_string(NAME,"L1D") or check_string(NAME,"L1I")){
          ...
          //@Hari not filling in lower level if it is not a reuse block in L2C
          if(MAKE_EXCLUSIVE_RIGID and check_string(NAME,"L2C") and l2c_block_access[victim_block_address]<2){
            //do nothing
          }else if(MAKE_REEXCLUSIVE_HASHED and check_string(NAME,"L2C") and check_reuse(victim_block_address)==0){
            //do nothing
          }else{
    ...
    if(MAKE_EXCLUSIVE and check_string(NAME,"L2C")){
        CACHE *LLC;
  ...
  while (writes_available_this_cycle > 0) {
    ...
    } else // MISS
    {
      ...
      if (handle_pkt.type == RFO && handle_pkt.to_return.empty()) {
        ...
        //@Hari evict victim to LLC from L2C;
        if(MAKE_EXCLUSIVE){
          //@Hari evicting non dirty packet and putting it in write queue of lower level
            ...
            if(check_string(NAME,"L2C") or check_string(NAME,"L1D") or check_string(NAME,"L1I")){
              ...
              writeback_packet.type = WRITEBACK;
              writeback_packet.is_dirty=block[set*NUM_WAY+way].dirty;
              writeback_packet.trip_cnt=handle_pkt.trip_cnt+1;
              ...
              if(MAKE_EXCLUSIVE_RIGID and check_string(NAME,"L2C") and l2c_block_access[victim_block_address]<2){
                ...
                //do nothing
                  ...
                  //To ensure the fill is after 140 events
                  /*if(MAKE_CRAZY and l2c_event_counter[set]-writeback_packet.l2c_limit<140){
                    WQ.pop_front();
                    WQ.push_back(writeback_packet);
                    return;
                  }*/
        ...
        //try inserting the victim in l2C writeback queue
        if(check_string(NAME,"LLC") and MAKE_CRAZY and l2c_block_access[victim_block_address]>30){
              ...
              if(handle_pkt.trip_cnt < 2 ){

                uint64_t victim_set=L2C->get_set(victim_address);
                l2c_event_counter[victim_set]++;

                PACKET writeback_packet;
                writeback_packet.fill_level = L2C->fill_level;
                writeback_packet.cpu = cpu;
                writeback_packet.address = victim_address;
                writeback_packet.data=block[set*NUM_WAY+way].data;  
                writeback_packet.instr_id = handle_pkt.instr_id;
                ...
                int result=L2C->add_wq(&writeback_packet);
        ...
        //To get Reuse of LLC eviction victims
        if(check_string(NAME,"LLC") and is_lru_victim and warmup_complete[handle_pkt.cpu] and success){
          ...
          L2C_access_counter++;
          l2c_set_access[L2C->get_set(victim_address)].push_back({(uint64_t)L2C_access_counter,victim_block_address,1});
          //cout<<__func__<<" "<<handle_pkt.instr_id<<" "<<L2C_access_counter<<" "<<victim_block_address<<endl;
        }

        if(MAKE_EXCLUSIVE  and success and check_string(NAME,"L2C")){
          //@Hari Invalidation the address to be filled is already present in LLC
          CACHE *LLC;
...
void CACHE::handle_read()
{
  while (reads_available_this_cycle > 0) {
    ...
    //@Hari to check for hit in case of exclusive/inclusive
    bool is_hit=(way<NUM_WAY);

    //@Hari block address to see address block behaviour
    uint64_t block_address=(handle_pkt.address>>LOG2_BLOCK_SIZE);

    //@Hari to check reuse distance
    if(NAME=="LLC" and warmup_complete[handle_pkt.cpu] and handle_pkt.type==LOAD){
      LLC_access_counter++;
      block_access_counter[block_address]++;

      //only if block has been accessed in past
      if(block_last_access_counter.find(block_address)!=block_last_access_counter.end()){
        
        int reuse_current=get_reuse(set,block_last_access_counter[block_address]);
        reuse_distance_counter[reuse_current]++;
        block_reuse_counter[block_address]+=reuse_current;
       
        }

      block_last_access_counter[block_address]=LLC_access_counter;
      set_access[set].push_back({block_address,LLC_access_counter});
    }


    if(check_string(NAME,"L2C") and warmup_complete[handle_pkt.cpu] and handle_pkt.type==LOAD){
      l2c_block_access[block_address]++;
      set_reuse(block_address);
    }
    ...
    //After getting evicted from LLC after how many unique access in l2c set is the block accessed
    if(check_string(NAME,"L2C") and warmup_complete[handle_pkt.cpu] and handle_pkt.type==LOAD ){
      
      l2c_event_counter[set]++;
      L2C_access_counter++;
      l2c_set_access[set].push_back({(uint64_t)L2C_access_counter,block_address,0});
    }
...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET& handle_pkt)
{
  ...
  if (!bypass) {
    ...
    if(MAKE_EXCLUSIVE and handle_pkt.is_dirty==0){
      fill_block.dirty=0;
    }
